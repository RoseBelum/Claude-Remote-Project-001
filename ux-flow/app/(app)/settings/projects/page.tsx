...
export default function ProjectsPage() {
  ...
  const estadoBadge: Record<Projecto['estado'], { bg: string; color: string }> = {
    activo:    { bg: 'rgba(215,229,187,0.7)', color: '#3f4b2c' },
    pausado:   { bg: 'rgba(255,222,173,0.6)', color: '#7a4000' },
    concluido: { bg: 'rgba(225,224,255,0.7)', color: '#585990' },
    arquivado: { bg: 'rgba(200,197,208,0.4)', color: '#777680' },
  }

  return (
    <div>
      <div className="flex items-center justify-between mb-5">
        <h2 className="text-lg font-extrabold text-[#181c23]">Projectos</h2>
        <button
          onClick={openCreate}
          ...
          <button
            ...
            className={`px-3 py-1.5 text-sm font-bold rounded-xl border transition-all ${
              estadoFilter === f
                ...
                : 'text-[#46464f] border-[#c8c5d0] hover:border-[#585990] hover:text-[#585990]'
            }`}
          >
      ...
      {loading ? (
        <p className="text-sm text-[#777680] font-medium">A carregar…</p>
      ) : filtered.length === 0 ? (
        <p className="text-sm text-[#777680] font-medium">Nenhum projecto encontrado.</p>
      ) : (
        <div className="glass rounded-[1.5rem] overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/60">
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider">Nome</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider">BU</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider hidden sm:table-cell">Área</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider">Estado</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider hidden lg:table-cell">Início</th>
                <th className="text-left py-3 px-4 text-xs font-bold text-[#46464f] uppercase tracking-wider hidden lg:table-cell">Fim</th>
            ...
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-b border-white/40 hover:bg-white/40 transition-colors">
                  <td className="py-3 px-4 font-semibold text-[#181c23]">{p.nome}</td>
                  <td className="py-3 px-4">
                    <span
                      className="inline-flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 rounded-full"
                      style={{ backgroundColor: (p.bu?.cor ?? '#585990') + '22', color: p.bu?.cor ?? '#585990' }}
                    >
                      {p.bu?.nome ?? '—'}
                    </span>
                  </td>
                  <td className="py-3 px-4 text-[#777680] font-medium hidden sm:table-cell">{p.area_bu ?? '—'}</td>
                  <td className="py-3 px-4">
                    <span
                  ...
                  <td className="py-3 px-4 text-[#777680] font-medium hidden lg:table-cell">{p.data_inicio ?? '—'}</td>
                  <td className="py-3 px-4 text-[#777680] font-medium hidden lg:table-cell">{p.data_fim ?? '—'}</td>
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2 justify-end">
                      <button onClick={() => openEdit(p)} className="text-xs font-bold text-[#585990] hover:underline">Editar</button>
                      {p.estado !== 'arquivado' && (
                        <button onClick={() => handleArchive(p)} className="text-xs font-bold text-[#777680] hover:underline">Arquivar</button>
                      )}
      ...
      {modalOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="glass rounded-[2rem] shadow-2xl w-full max-w-md p-7">
            <h3 className="text-base font-extrabold text-[#181c23] mb-5">
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-[#46464f] uppercase tracking-widest mb-2">Área BU</label>
                <input
                  type="text"
                  value={form.area_bu}
              ...
              <div>
                <label className="block text-xs font-bold text-[#46464f] uppercase tracking-widest mb-2">Estado</label>
                <select
                  value={form.estado}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-[#46464f] uppercase tracking-widest mb-2">Data início</label>
                  <input
                    type="date"
                ...
                <div>
                  <label className="block text-xs font-bold text-[#46464f] uppercase tracking-widest mb-2">Data fim</label>
                  <input
                    type="date"
                    value={form.data_fim}
            ...
            <div className="flex justify-end gap-2 mt-6">
              <button
                ...
                className="px-4 py-2.5 text-sm font-bold text-[#46464f] bg-white/60 border border-[#c8c5d0] rounded-xl hover:bg-white transition-all"
              >
                Cancelar
              </button>
              <button
